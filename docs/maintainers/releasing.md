...
3. Confirm interval metrics remain pure, immutable, nonnegative, and independent of admission behavior.
4. Update the project and runtime versions to `0.4.0`.
5. Add a dated `0.4.0` changelog section while retaining all earlier release history.
...
9. Create and push the annotated `v0.4.0` tag.
...
```bash
git tag -a v0.4.0 -m "Bulklink 0.4.0"
...
For the stable `0.4.x` line, the release also verifies the documented public exports,
...
- runtime use of relative and absolute admission limits, interval metrics, resizing, diagnostics, events, shutdown, and the registry;
...
Patch releases preserve the documented `0.4.x` contract. A future pre-1.0 minor release
